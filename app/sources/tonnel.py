...
class TonnelSource(MarketSource):
    name = "Tonnel"

    def __init__(self, settings: Settings, http_client: MarketHTTPClient | None = None) -> None:
        self.enabled = settings.tonnel_enabled
        self.base_url = settings.tonnel_base_url.strip()
        self.api_key = settings.tonnel_api_key.strip()
    ...
    async def _request(self, path: str) -> dict | list | None:
        ...
        if not self.base_url:
            logger.info("Tonnel endpoint is not configured/implemented yet")
            return None
        ...
        try:
            ...
            logger.warning("Tonnel source error: %s", exc)
            return None

    async def get_collection_floor(self, collection: str) -> MarketFloor | None:
        # TODO: add endpoint when official docs are confirmed.
        logger.info("Tonnel collection floor endpoint is not configured/implemented yet")
        return None

    async def get_trait_floor(self, collection: str, trait_type: str, trait_value: str) -> MarketFloor | None:
        logger.info("Tonnel trait floor endpoint is not configured/implemented yet")
        return None

    async def get_recent_sales(self, collection: str, limit: int = 20) -> list[SaleSchema]:
        logger.info("Tonnel recent sales endpoint is not configured/implemented yet")
        return []

    async def get_similar_listings(
        self, collection: str, attributes: list[GiftAttributeSchema], limit: int = 20
    ) -> list[ListingSchema]:
        logger.info("Tonnel similar listings endpoint is not configured/implemented yet")
        return []

    async def search_underpriced(self, collection: str, filters: dict) -> list[ListingSchema]:
        logger.info("Tonnel underpriced search endpoint is not configured/implemented yet")
        return []
